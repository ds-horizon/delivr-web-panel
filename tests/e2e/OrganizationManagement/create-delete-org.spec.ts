...
test.describe('Organization Management - Create & Delete', () => {
  ...
  test('Org 1: Create Organization with App', async ({ page }) => {
    ...
    await page.waitForTimeout(2000);
    console.log('✅ Opened Create Organization modal');
    ...
    console.log(`✅ Filled initial app name: ${appName}`);
    ...
    console.log('✅ Success notification: "Organization created successfully!"');
    ...
    console.log(`✅ Organization "${orgName}" found in list`);
    
    console.log('✅ Test passed - Organization created successfully');
  });

  test('Org 2: Delete Organization (Owner Only)', async ({ page }) => {
    test.setTimeout(60000); // 60 seconds
    console.log('🚀 Test: Delete Organization');
    ...
    console.log('✅ Found test-org-1 (user is owner)');
    ...
    if (await menuButton.isVisible().catch(() => false)) {
      ...
      console.log('✅ Organization removed from list');
      
      console.log('✅ Test passed - Organization deleted successfully');
    } else {
      console.log('⚠️ Delete option not found');
      console.log('ℹ️  Note: Delete may only be available for org owners');
      console.log('ℹ️  Or delete might be in a different location (settings page, etc.)');
    }
  });

  test('Org 3: Cancel Organization Deletion', async ({ page }) => {
    console.log('🚀 Test: Cancel Organization Deletion');
    ...
    if (await menuButton.isVisible().catch(() => false)) {
      ...
      await deleteOption.click();
      await page.waitForTimeout(2000);
      console.log('✅ Opened delete confirmation');
      ...
      console.log('✅ Organization still in list (not deleted)');
      
      console.log('✅ Test passed - Cancel works correctly');
    } else {
      console.log('⚠️ Delete option not available - skipping test');
    }
  });
});
